            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            <a href="{{ route('transaksis.show', $transaksi->idtransaksi) }}" class="btn btn-info">
                    ...
                    <div class="card-body">
                        ...
                        <!-- FORM UPDATE -->
                        <form method="POST" action="{{ route('transaksis.update', $transaksi->idtransaksi) }}">
                            @csrf
                            @method('PUT')
                                ...
                                <select class="form-select @error('idpesanan') is-invalid @enderror" id="idpesanan" name="idpesanan" required>
                                    ...
                                    @foreach($pesanans as $pesanan)
                                        <option value="{{ $pesanan->idpesanan }}"
                                            ...
                                            {{ $pesanan->pelanggan->namapelanggan }} - ({{ $pesanan->detailPesanans->count() }} item)
                                        </option>
                                    @endforeach
                                </select>
                                @error('idpesanan')
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" class="form-control @error('total') is-invalid @enderror"
                                        id="total" name="total" value="{{ old('total', $transaksi->total) }}" min="0" required>
                                </div>
                                @error('total')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" class="form-control @error('bayar') is-invalid @enderror"
                                        id="bayar" name="bayar" value="{{ old('bayar', $transaksi->bayar) }}" min="0" required>
                                </div>
                                @error('bayar')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <div class="form-text">Jumlah bayar harus ≥ total harga.</div>
                            </div>
                        ...
                        <!-- Hidden Form Delete -->
                        <form id="delete-form" method="POST" action="{{ route('transaksis.destroy', $transaksi->idtransaksi) }}" style="display:none;">
                            @csrf
                            @method('DELETE')
                        </form>
                    </div>
